export const webDev = {
    title: 'Web Development',
    icon: '/Portfolio/hosting.png',
    Skills: [
        {
            imgPath: '/Portfolio/react.png',
            name: 'React',
            progress: 85,
            imgStyle: {borderRadius: '13px', border: '2px solid #00d8ff' },
            documentation: 'https://reactjs.org/docs/getting-started.html'
        },
        {
            imgPath: '/Portfolio/nodejs.png',
            name: 'NodeJS',
            progress: 75,
            imgStyle: {
                backgroundColor: 'white',
                borderRadius: '20px',
                border: '3px solid #026e00',
                padding: '5px'
            },
            documentation: 'https://nodejs.org/en/docs/'
        },
        {
            imgPath: '/Portfolio/Express.png',
            name: 'Express',
            progress: 70,
            imgStyle: { borderRadius: '22px', backgroundColor: '#343434',paddingTop:'5px', border: '2px solid #fff' },
            documentation: 'https://expressjs.com/en/starter/installing.html'
        },
        {
            imgPath: '/Portfolio/mySql.png',
            name: 'MySql',
            progress: 80,
            imgStyle: { backgroundColor: 'white', borderRadius: '20px', border: '3px solid #01618b', padding:'2px' },
            documentation: 'https://dev.mysql.com/doc/'
        },
        {
            imgPath: '/Portfolio/mongodb.png',
            name: 'MongoDB',
            progress: 60,
            documentation: 'https://docs.mongodb.com/'
        },
        {
            imgPath: '/Portfolio/GraphQL.png',
            name: 'GraphQL',
            progress: 70,
            imgStyle: { backgroundColor: 'white', borderRadius: '20px', padding: '5px', border: '2px solid #e10098' },
            documentation: 'https://graphql.org/learn/'
        },
        {
            imgPath: '/Portfolio/Socket.io.png',
            name: 'Socket.io',
            progress: 85,
            imgStyle: { backgroundColor: 'white', borderRadius: '20px', padding: '5px', border:'3px solid black' },
            documentation: 'https://socket.io/docs/v4/'
        },
        {
            imgPath: '/Portfolio/tailwind.jpg',
            name: 'Tailwind',
            progress: 80,
            imgStyle: { borderRadius: '13px', border: '3px solid #00d8ff' },
            documentation: 'https://tailwindcss.com/docs'
        },
        {
            imgPath: '/Portfolio/html.png',
            name: 'HTML',
            progress: 80,
            documentation: 'https://developer.mozilla.org/en-US/docs/Web/HTML'
        },
        {
            imgPath: '/Portfolio/css.png',
            name: 'CSS',
            progress: 93,
            documentation: 'https://developer.mozilla.org/en-US/docs/Web/CSS'
        }
    ]
}

export const programmingLanguages = {
    title: 'Programming Languages',
    icon: '/Portfolio/coding.png',
    Skills: [
        {
            imgPath: '/Portfolio/python.png',
            name: 'Python',
            progress: 90,
            documentation: 'https://docs.python.org/3/'
        },
        {
            imgPath: '/Portfolio/c-.png',
            name: 'C++',
            progress: 90,
            documentation: 'https://en.cppreference.com/w/'
        },
        {
            imgPath: '/Portfolio/C.png',
            name: 'C',
            progress: 95,
            documentation: 'https://en.cppreference.com/w/c'
        },
        {
            imgPath: '/Portfolio/js.png',
            name: 'JavaScript',
            progress: 65,
            documentation: 'https://developer.mozilla.org/en-US/docs/Web/JavaScript'
        }
    ]
}

export const tools = {
    title: 'Tools',
    icon: '/Portfolio/technology.png',
    Skills: [
        {
            imgPath: '/Portfolio/social.png',
            name: 'Git',
            progress: 70,
            documentation: 'https://git-scm.com/doc'
        },
        {
            imgPath: '/Portfolio/Postman.png',
            name: 'Postman',
            progress: 73,
            documentation: 'https://learning.postman.com/docs/getting-started/introduction/'
        },
        {
            imgPath: '/Portfolio/github.png',
            name: 'GitHub',
            progress: 75,
            documentation: 'https://docs.github.com/en'
        }
    ]
}