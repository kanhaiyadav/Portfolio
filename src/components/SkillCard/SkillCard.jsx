...
const SkillCard = ({ imgPath, name, progress, exp, imgStyle, ...otherProps }) => {
    ...
    return (
        <div className="skill-card flex gap-3 w-[400px] h-[130px] p-3 rounded-xl shadow-[6px_6px_0px_3px_rgba(0,0,0,1)]" {...otherProps}>
            <div className="h-full" style={imgStyle}>
                <img src={imgPath} alt={name} className="h-full rounded-xl" />
            </div>
            <div className="flex-1">
                <h2 className="text-white text-4xl font-bold font-Poppins">{name}</h2>
                <p className="text-white text-lg font-Playwrite1">Started {exp} years ago</p>
